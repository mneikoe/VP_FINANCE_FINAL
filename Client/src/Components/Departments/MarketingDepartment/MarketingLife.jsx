...
const KIND_OF_FORM = "life_insurance"; // 🔒 FIXED FOR THIS PAGE
...
const MarketingLife = () => {
  ...
  return (
    <div className="p-4">
      {/* Header */}
      <div className="flex navbar justify-center mb-6 border-b border-gray-300">
        <button
          className="px-4 py-2 bg-blue-600 text-white rounded"
          onClick={() => setOpen(true)}
        >
          + Add Marketing Life Insurance Form
        </button>
      </div>
      ...
      <table className="w-full border text-sm">
        <thead className="bg-gray-100">
          <tr className="text-gray-500">
            ...
            <th className="border px-2 py-1">Form Type</th>
            <th className="border px-2 py-1">Form Name</th>
          ...
          {loading ? (
            ...
            forms.map((f) => (
              <tr key={f._id}>
                ...
                <td className="flex justify-center items-center border px-2 py-1 text-center gap-2">
                  <button
                    className="text-blue-600 border rounded px-2 py-1 bg-blue-200"
                    onClick={() => handleEdit(f)}
                  >
                    Edit
                  </button>
                  <button
                    className="text-red-600 bg-red-200 rounded px-2 py-1"
