...
//   // Form state with updated depart as array (SINGLE ROLE for service)
//   const [formData, setFormData] = useState({
//     cat: "",
//     sub: "",
//     depart: [], // ✅ SINGLE role array for service
//     name: "",
//     type: "service", // ✅ FIXED: Service type
//     estimatedDays: 1,
...
//     if (name === "depart") {
//       // ✅ Handle SINGLE selection for depart array (service)
...
//       formDataToSend.append("type", "service"); // ✅ Service type
...
//       // ✅ Append depart as array (SINGLE role for service)
...
//         }, 1500);
//       } else {
//         setError(response.data.message || "Failed to save service task");
//       }
//     } catch (error) {
...
//             <div className="row">
//               {/* ✅ UPDATED: Employee Role (SINGLE SELECT for Service) */}
//               <div className="col-md-6">
//                 <div className="form-group">
...
//                     Service tasks can be assigned to single role only
//                   </small>
...
const AddTaskService = ({ on, data, onSuccess }) => {
  ...
  const flat = data
    ? {
      ...
      productName: data?.cat?.name,
      descText: data?.descp?.text,
      descImage: data?.descp?.image,
    }
    : null;
  ...
  } = useSelector((state) => state.ServiceTask);
  ...
  // Set form data when editing
  useEffect(() => {
    if (!data) return;                 // 🔥 HARD STOP in ADD mode
    if (!flat || isEditInitialized.current) return;

    setFormData({
      cat: flat.cat?._id || "",
      sub: "",
      depart: Array.isArray(flat.depart) ? flat.depart : [],
      name: flat.name || "",
      type: "service",
      ...
      sms_descp: flat.sms_descp || "",
      whatsapp_descp: flat.whatsapp_descp || "",
      checklists: flat.checklists?.length ? flat.checklists : [""],
      formChecklists:
  ...
  useEffect(() => {
    if (data) return; // edit mode → do nothing
  ...
  useEffect(() => {
    if (!flat || !formData.cat) return;

    // ✅ only set once, and only if sub is empty
    if (formData.sub) return;

    const match = filteredCompanies.find(
      (c) => c.companyName === flat.sub
    );

    if (match) {
      setFormData((prev) => ({
        ...prev,
        sub: match.companyName,
      }));
    }
  }, [flat, formData.cat, filteredCompanies]); // ✅ FIXED DEPENDENCIES
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const validFormChecklists = formData.formChecklists
        .filter(item => item.name && item.name.trim() !== "")
        .map(item => ({ name: item.name }));

      if (validFormChecklists.length > 0) {
        formDataToSend.append(
          "formChecklists",
          JSON.stringify(validFormChecklists)
        );
      }
      ...
      if (data) {
        ...
        await dispatch(createServiceTask(formDataToSend)).unwrap();
      }
        ...
        >
          <div className="card-body">
            ...
            <div className="row">
              ...
              <div className="col-md-6">
                <div className="form-group">
                  <label className="font-weight-bold">Reward Points</label>
                  <input
                    type="number"
                    name="rewardPoints"
                    className="form-control"
                  ...
                  />
                  <small className="text-muted">
                    Points awarded after completing this service task
                  </small>
                </div>
              </div>
            ...
            <div className="nav-tabs-custom mt-4">
              ...
              <div className="tab-content p-3 border border-top-0 rounded-bottom">
                ...
                {/* Tab 5: WhatsApp Templates */}
                <div
                  className={`tab-pane fade ${activeTab === "tab_5" ? "show active" : ""
                    }`}
                >
                  <div className="card">
                    <div className="card-header bg-light">
                ...
                {/* Tab 6: Download Forms */}
                <div
                  className={`tab-pane fade ${activeTab === "tab_6" ? "show active" : ""
                    }`}
                >
                  <div className="card">
                    <div className="card-header bg-light d-flex justify-content-between align-items-center">
