...
const Service = () => {
  ...
  // Fetch single service task by ID
  const handleEdit = async (id) => {
    const resultAction = await dispatch(fetchServiceTaskById(id));

    console.log("EDIT RESULT ACTION:", resultAction);
    console.log("EDIT PAYLOAD:", resultAction.payload);

    if (fetchServiceTaskById.fulfilled.match(resultAction)) {
      setUpdate(resultAction.payload);
      setActiveTab("add");
    }
  };
          ...
          <div className="card card-outline">
            ...
            <div className="card-body">
              ...
              {activeTab === "add" && (
                <div>
                  <AddTaskService
                    key={update?._id || "new"}   // now works
                    on={setActiveTab}
                    data={update}               // task object
                    onSuccess={() => {
                      setUpdate(null);
                      setActiveTab("view");
                      dispatch(fetchAllServiceTasks());
