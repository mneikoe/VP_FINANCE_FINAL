...
const AddTaskMarketing = ({ data, onSuccess }) => {
  ...
  const {
    ...
    error = null,
    successMessage = null,
  } = useSelector((state) => state.MarketingTask || {});

  // Form state with updated depart as array (SINGLE ROLE for marketing)
  const [formData, setFormData] = useState({
    cat: "",
    sub: "",
    depart: "",          // ✅ SAME AS COMPOSITE (string, not array)
    name: "",
    type: "marketing",   // fixed
    rewardPoints: 0,
    estimatedDays: 1,
    templatePriority: "medium",
    descp: { text: "", image: null },
  ...
  //       cat: flat?.cat?._id || "",
  //       sub: flat?.sub || "",
  //       depart: Array.isArray(flat?.depart)
  //         ? flat.depart[0]
  //         : flat?.depart || "",
  //       name: flat?.name || "",
  //       type: flat?.type || "marketing", // ✅ Always marketing
  ...
  // }, [flat]);
  useEffect(() => {
    if (flat && !isEditInitialized.current) {
      setFormData((prev) => ({
        ...prev,
        cat: flat?.cat?._id || "",
        sub: flat?.sub || "",
      ...
      isEditInitialized.current = true; // 🔒 lock it
    }
  }, [flat]);
  ...
  const handleSubmit = async (e) => {
    ...
    formData.formChecklists.forEach((item, index) => {
      if (item.downloadFormUrl instanceof File) {
        formDataToSend.append(`downloadFormUrl[${index}]`, item.downloadFormUrl);
      }
      if (item.sampleFormUrl instanceof File) {
        formDataToSend.append(`sampleFormUrl[${index}]`, item.sampleFormUrl);
      }
    });
            ...
            <div className="row">
              ...
              <div className="col-md-6">
                <div className="form-group">
                  ...
                  />
                  <small className="text-muted">
                    Points awarded after completing this marketing task
                  </small>
                </div>
              </div>
