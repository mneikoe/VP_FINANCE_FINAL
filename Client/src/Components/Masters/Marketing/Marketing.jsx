...
//     if (
//       window.confirm("Are you sure you want to delete this marketing task?")
//     ) {
//       try {
//         const response = await axios.delete(`/api/Task/marketing/${id}`);
//         if (response.data.success) {
//           alert("Marketing task deleted successfully");
...
//                       </div>
//                       <p className="mt-3">Loading marketing tasks...</p>
//                     </div>
//                   ) : error ? (
...
const Marketing = () => {
  ...
  const handleEdit = async (id) => {
    setActiveTab("add");
    const result = await dispatch(fetchMarketingTaskById(id));
    setUpdate(result.payload); // ✅ actual task
  };
          ...
          <div className="card card-outline">
            <div style={{ backgroundColor: "#ECECEC" }} className="card-header">
              <Tabs
                ...
                onSelect={(k) => {
                  setActiveTab(k);
                  if (k === "add") setUpdate(null);   // 🔥 MAIN FIX
                }}
                className="mb-3"
              >
                <Tab eventKey="view" title={<b>View Marketing Tasks</b>} />
                        ...
                        <tbody>
                          {tasks.map((task, index) => (
                            <tr key={task._id || index}>
                              ...
                              <td>
                                <span
                                  className={`badge bg-${task.templatePriority === "urgent"
                                    ? "danger"
                                    : task.templatePriority === "high"
                                      ? "warning"
                                      : task.templatePriority === "medium"
                ...
                <div>
                  <AddTaskMarketing
                    ...
                    onSuccess={() => {
                      setUpdate(null);
                      setActiveTab("view");
                      setRefreshKey(prev => prev + 1); // 🔥 FORCE FETCH
                    }}
        ...
        <Modal.Body>
          ...
          <div className="mb-3">
            <span className="badge bg-success fs-6">
              Reward Points: {currentTask?.rewardPoints ?? 0}
            </span>
          </div>
