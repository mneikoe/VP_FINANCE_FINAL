...
const Addtask = ({ on, data, onSuccess }) => {
  ...
  useEffect(() => {
    if (flat && !isInitialized) {
      ...
      setIsInitialized(true); // 🔥 MOST IMPORTANT LINE
    }
  }, [flat, isInitialized]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const validChecklists = formData.checklists.filter(
        (item) => item.trim() !== ""
      );

      // Add formChecklists as JSON string
      const validFormChecklists = formData.formChecklists.filter(
        (item) => item.name && item.name.trim() !== ""
      );

      validFormChecklists.forEach((item, index) => {
      ...
      // 🔥 EXACT FIX
      const isEdit = Boolean(data?.task?._id);

      if (isEdit) {
        await dispatch(
          updateCompositeTask({
            id: data.task._id,   // ✅ ID must exist
            formData: formDataToSend,
          })
        );
      } else {
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {["low", "medium", "high", "urgent"].map((priority) => (
                      <button
                        ...
                        className={`px-3 py-2 rounded text-sm font-medium transition-colors ${formData.templatePriority === priority
                          ? priority === "urgent"
                            ? "bg-red-50 text-red-700 border border-red-300"
                            : priority === "high"
                              ? "bg-orange-50 text-orange-700 border border-orange-300"
