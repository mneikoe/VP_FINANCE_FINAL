...
const FamilyMembersForm = ({ clientId, clientData, onClientCreated, familyDetail, setFamilyDetail }) => {
  ...
  const defaultMember = (isSelf = false, data = {}) => ({
    ...
    title: isSelf
      ? (familyDetail?.salutation || data.personalDetails?.salutation || data.title || "")
      : (data.title || ""),
  ...
  const handleSubmit = async (e) => {
    ...
    const isValid = familyMembers.every((member) => {
      ...
      if (!member.dobActual) return false;

      if (member.includeHealth) {
        if (
      ...
      if (member.relation !== "Self" && !member.relation) {
        return false;
      }
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (updateFamilyMember.fulfilled.match(result)) {
          ...
          setFamilyDetail(updatedMembers);
            ...
            <Col md={2}>
              <Form.Group controlId={`title-self`}>
                ...
                <Form.Control
                  ...
                  readOnly
                  value={
                    familyDetail?.salutation ||
                    clientData?.personalDetails?.salutation ||
                    selfMember.title ||
                    "N/A"
                  }
                />
            ...
            <Col md={3}>
              <Form.Group controlId={`contact-self`}>
                ...
                <Form.Control
                  ...
                  readOnly
                  value={
                    familyDetail?.mobileNo ||
                    clientData?.personalDetails?.mobileNo ||
                    selfMember.contact ||
                    "N/A"
                  }
                />
