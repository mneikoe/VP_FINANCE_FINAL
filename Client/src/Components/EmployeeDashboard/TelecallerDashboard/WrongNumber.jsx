...
const WrongNumber = () => {
  ...
  // Filter for latest task = "Wrong Number" and exclude if forwarded after task
  const data = suspects
    .filter((suspect) => {
      ...
      // Status must match
      if (lastTask.taskStatus !== "Wrong Number") return false;
  ...
  return <LeadsTableLayout title="Wrong Number" data={data} columns={columns} />;
};
